-- 1. Delete ALL instances of the "Additional Instructions" field
DELETE FROM task_fields 
WHERE field_label ILIKE '%additional%' 
   OR field_name ILIKE '%additional%'
   OR field_label ILIKE '%instructions%';

-- 2. Update the default app generation function
-- We use CREATE OR REPLACE to avoid dropping the function (which causes the trigger dependency error)
