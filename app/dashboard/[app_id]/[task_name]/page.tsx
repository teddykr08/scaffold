...
export default function TaskEditor({ params }: { params: { app_id: string, task_name: string } }){
  ...
  useEffect(()=>{
    async function load(){
      ...
      if(pData.success && pData.templates && pData.templates.length) setTemplate(pData.templates[0].template);
    }
    load();
  },[app_id, task_name]);
  ...
  function insertAtCursor(v:string){
    ...
    setTemplate(newVal);
    setTimeout(()=>{ ta.focus(); ta.selectionStart = ta.selectionEnd = start + v.length; },10);
  }

  async function saveTemplate(){
    await fetch('/api/prompt-templates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({app_id,task_name,template})});
  ...
  async function moveField(id:string, dir:'up'|'down'){
    ...
    const arr = [...fields];
    const a = arr[idx]; arr[idx]=arr[to]; arr[to]=a;
    // update order values
    ...
    if(data.success) setFields(arr.map((f,i)=>({...f,order:i+1})));
  }
    ...
    <main className="min-h-screen bg-white p-6">
      <div className="max-w-6xl mx-auto grid grid-cols-3 gap-6">
        <div className="col-span-3">
          <button onClick={()=>router.push(`/dashboard/${app_id}`)} className="text-sm text-gray-600">← Back to Projects</button>
        ...
        <div className="col-span-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={hasForm} onChange={async (e)=>{ setHasForm(e.target.checked); await fetch('/api/tasks',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({app_id, name:task_name, has_form: e.target.checked})}); }} />
                  <span>Enable Form</span>
                </label>
              </div>
            </div>
            {!hasForm && (
              <div className="mt-3 text-sm text-gray-600">This task uses only <<fixed>> - no user form</div>
            )}
          </div>
          ...
          <div className="rounded-lg border p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={saveTemplate} className="px-3 py-1 bg-black text-white rounded">Save Template</button>
            </div>

            <div className="grid grid-cols-12 gap-4">
              <div className="col-span-8">
                <textarea ref={textareaRef} value={template} onChange={e=>setTemplate(e.target.value)} className="w-full h-64 font-mono p-2 border rounded" />
              </div>
